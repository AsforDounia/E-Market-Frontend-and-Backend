...
class NotificationService extends EventEmitter {
    ...
    async handlePublishProduct(data, next) {
        try {
            if (!data.productId || !data.sellerId || !data.title) {
                throw new AppError(
                    "Missing required product data for notification (productId, sellerId, title)",
                    400
                );
            }
            ...
            console.log(
                "The Product <",
                data.title,
                "> is published and the notification sent to all subscribers."
            );
        } catch (error) {
            next(error);
        }
    ...
    async handleOrderUpdated(data, type) {
        try {
            ...
            await UserNotification.insertMany(userNotifications);
            console.log(
                `Order ${type === "ORDER_UPDATED" ? "updated" : "cancelled"} notification sent to ${sellers.length} sellers`
            );
        } catch (error) {
